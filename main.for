...
C**** CLEAR CARD AND CARDT                                              MAI00037
 ...
 57   CONTINUE                                                          MAI00041
      MCOM=0                                                            MAI00042
C     it is not sufficient to add END=, need to check IOSTAT            ***
 6100 READ(IRC,51, IOSTAT=IO, END=8210) CARD                            MAI00043
      IF (IO.NE.0) GOTO 8210                                            ***
C*-*-                                                                   MAI00044
C*-*- CALL FOR A POSIBLE NON-COMPATIBLE ROUTINE -IFEOF(IRC)-            MAI00045
C*-*-     ----CHECK IT----       ----CHECK IT----                       MAI00046
C*-*-                                                                   MAI00047
C     IF(IFEOF(IRC).EQ.-1) GOTO 8210                                    MAI00048
...
C*****COMMA PRESENT                                                     MAI00505
 597  IFR=LOC+2                                                         MAI00506
      ...
      GOTO 525                                                          MAI00508
C                                                                       MAI00509
 600  IF((CARDP(IBEGST).NE.ALPH(7)).OR.(CARDP(IBEGST+2).NE.ALPH(20)))   MAI00510
     1 GOTO 650                                                         MAI00511
...
C*****LIST DATA AFTER END STATEMENT                                     MAI01034
 ...
 3011 READ(IRC,51,END=3020) CARD                                        MAI01040
C     IF(IFEOF(IRC).EQ.-1) GOTO 3020                                    MAI01041
C*-*-                                                                   MAI01042
